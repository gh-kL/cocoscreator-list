...
* `PAGE`模式下，目前是靠`pageDistance`变量来控制`翻页响应距离`，如果滚动窗口很小，玩家拖动时一次性能拖（翻）1页以上，那就会存在问题。
...
> 设为循环列表时，Content的cc.Layout边缘距离（top/bottom/left/right）要设置成与spacingX/Y（间距）一样。（因为在循环列表中，边距=间距）。
