...
### 2019/12/26（目前只更了JS版，等TS版的小伙伴骚安勿躁）

- 弃用手动计算`customSize`，现在要实现`动态宽/高`，请给`模板Item`挂载`ListItem组件`，并勾选 adaptiveSize 属性即可，全自动，再也不用算`customSize`了，使用方法请看示例项目中的`TestAdaptive`场景。惊不惊喜，意不意外。
