 ...
 *      3、各种RIGHT_TO_LEFT的布局都会有问题，官方的cc.ScrollView是不支持RIGHT_TO_LEFT滚动的（item数量过少，就会导致content错位）。而本组件是配合cc.ScrollView去写的，所以也不支持。
...
cc.Class({
    ...
    _init() {
        ...
        if (!this._scrollView) {
            console.error(this.node.name + '没有ScrollView组件！');
            return;
        }
        this.content = this._scrollView.content;
        if (!this.content) {
            console.error(this.node.name + '的ScrollView没有设置Content！');
            return;
        }

        this._layout = this.content.getComponent(cc.Layout);

        // this._selectionMode = 0;                        //选择模式
        // this._lastSelectedIndex = 0;                    //最后一个选择idx
        ...
        this.setTemplateItem(this.tmpPrefab ? this.tmpPrefab.data : this.tmpNode);
    ...
    _resizeContent() {
        ...
        // console.log('_resizeContent()  numItems =', this._numItems, '，content =', this.content);
    },
    //滚动进行时...
    _onScrolling(ev) {
        ...
        if (this._virtual) {
            ...
            if (this._forceUpdate ||
                ...
                if (this.frameByFrameRenderNum > 0) { //逐帧渲染
                    ...
                    // console.log('List Display Data I::', this.displayData);
                } else { //直接渲染
    ...
    scrollTo(listId, timeInSecond, offset, overStress) {
        ...
        t.scheduleOnce(() => {
            ...
            //console.log('2222222222', this._adheringBarrier)
            if (overStress) {
    ...
    _calcNearestItem() {
        ...
        let firstVisibleListId = this.firstListId;
        let bool = true;
        if (!this._virtual)
            this._calcViewPos();
        for (let n = 0; n < this.content.childrenCount; n += this._colLineNum) {
            if (this._virtual) {
            ...
            let d = this.getIntersection(p1, p2, p3, p4);
            //console.log(data.id, '   |   ', p1, p2, p3, p4, '   |  ', d, distance);
            ...
            if (d > 0 && d > distance && Math.abs(distance - d) > .5) {
                ...
                if (data.id == firstVisibleListId) {
                    ...
                    if (this._virtual) {
                        ...
                        if (this.customSize) {
                            let cs = this.customSize[firstVisibleListId];
                            if (cs) {
                                size = cs;
                            }
                        }
        ...
        //console.log('this.nearestListId =', this.nearestListId);
    },
    /**
     * 获取两组数字区间的交集差
     */
